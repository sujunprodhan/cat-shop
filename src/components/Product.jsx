...
const Product = async ({ page = 1, search = '', sort = '' }) => {
  const currentPage = Number(page);
  const { products, pages } = await getProducts(currentPage, 6, search, sort);
      ...
      {pages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8 py-10 border-t border-white/5">
          <div className="flex items-center gap-4">
            <Link
              href={`?page=${currentPage - 1}${search ? `&search=${search}` : ''}${sort ? `&sort=${sort}` : ''}`}
              scroll={false}
            ...
            <div className="flex items-center gap-2 px-6">
              {[...Array(pages)].map((_, i) => {
                ...
                return (
                  <Link
                    key={pageNum}
                    href={`?page=${pageNum}${search ? `&search=${search}` : ''}${sort ? `&sort=${sort}` : ''}`}
                    scroll={false}
